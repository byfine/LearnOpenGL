//使用两个fragment着色器，使用不同的VAO(和VBO)创建同样的2个三角形，每个三角形的数据不同
...
void ComplieShader()
{
	...
	// Create program Orange, attach shader to it, and link it
	shaderProgramOrange = glCreateProgram();
	glAttachShader(shaderProgramOrange, vertexShader);
	glAttachShader(shaderProgramOrange, fragmentShaderOrange);
	glLinkProgram(shaderProgramOrange);

	// Create program Yellow, attach shader to it, and link it
